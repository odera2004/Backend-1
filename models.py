...
class User(db.Model):
    ...
    is_admin = db.Column(db.Boolean, default=False)  # Admin for managing the system
...
class Guard(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    shift_start = db.Column(db.String, nullable=False)  # Example: "08:00 AM"
    shift_end = db.Column(db.String, nullable=False)  # Example: "04:00 PM"
...
class WorkOrder(db.Model):
    ...
    status = db.Column(db.String(64), default="Pending")  # Could be "Pending", "In Progress", "Completed"
